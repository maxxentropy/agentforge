...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Run tests that require real API access (requires ANTHROPIC_API_KEY)",
    )
...
def pytest_collection_modifyitems(config, items):
    """Skip real_api tests unless --run-real-api is passed."""
    if config.getoption("--run-real-api"):
        # --run-real-api given: do not skip real_api tests
        return
